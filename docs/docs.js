...
/*
const createCompressorKnee = (svg) => {
  const innerWidth = Number(svg.getAttribute('width')) - padding * 2;
  const innerHeight = Number(svg.getAttribute('height')) - padding * 2;

  const width = innerWidth / 2 - padding;

  const renderGraph = (offset) => {
    const rect = document.createElementNS(xmlns, 'rect');
    ...
    ['0.00', '0.25', ' 0.50', '0.75', '1.00'].forEach((x, index) => {
      ...
      text.setAttribute('x', ((width / 4) * index + offset + padding).toString(10));
      text.setAttribute('y', (padding + innerHeight + 20).toString(10));
      text.setAttribute('text-anchor', 'middle');
      text.setAttribute('stroke', 'none');
      text.setAttribute('fill', baseColor);
      ...
      xRect.setAttribute('x', ((width / 4) * index + offset + padding).toString(10));
      xRect.setAttribute('y', padding.toString(10));
      xRect.setAttribute('width', lineWidth.toString(10));
      xRect.setAttribute('height', innerHeight.toString(10));
      xRect.setAttribute('stroke', 'none');
    ...
    const createLinearPath = () => {
      ...
      path.setAttribute('d', `M${offset + padding} ${padding + innerHeight} L${offset + padding + width} ${padding}`);
      path.setAttribute('stroke', alphaWaveColor);
      path.setAttribute('fill', 'none');
      path.setAttribute('stroke-width', lineWidth.toString(10));
      path.setAttribute('stroke-linecap', lineCap);
  ...
  const renderCompressionCurve = (offset, isSoft) => {
    const path = document.createElementNS(xmlns, 'path');

    if (isSoft) {
      path.setAttribute(
        'd',
        `M${offset + padding} ${padding + innerHeight} L${offset + padding + width / 2} ${padding + innerHeight / 2} Q${offset + padding + width - 120} ${padding + (3 / 8) * innerHeight - 4} ${offset + padding + width - 60} ${padding + (3 / 8) * innerHeight - 8} T${offset + padding + width} ${padding + (3 / 8) * innerHeight - 12}`
      );
    } else {
      path.setAttribute(
        'd',
        `M${offset + padding} ${padding + innerHeight} L${offset + padding + width / 2} ${padding + innerHeight / 2} L${offset + padding + width} ${padding + (3 / 8) * innerHeight}`
      );
    }
  ...
  renderGraph(0);
  renderCompressionCurve(0, false);

  const offset = innerWidth / 2 + padding;

  renderGraph(offset);
  renderCompressionCurve(offset, true);
};
*/
...
const animateWindowFunctions = (svgTime, svgSpectrum) => {
  ...
  const drawOscillator = () => {
    ...
    switch (windowFunction) {
      ...
      case 'hanning': {
        for (let n = 0; n < fftSize; n++) {
          windowFunctions[n] = 0.5 - 0.5 * Math.cos((2 * Math.PI * n) / fftSize);
        }
      ...
      case 'hamming': {
        for (let n = 0; n < fftSize; n++) {
          windowFunctions[n] = 0.54 - 0.46 * Math.cos((2 * Math.PI * n) / fftSize);
        }
      ...
      case 'blackman': {
        ...
        for (let n = 0; n < fftSize; n++) {
          windowFunctions[n] = (1 - alpha) / 2 - 0.5 * Math.cos((2 * Math.PI * n) / fftSize) + (alpha / 2) * Math.cos((4 * Math.PI * n) / fftSize);
        }
...
const animateAmplitudeSpectrumToSVG = (svg, button, displayGraph, displayText) => {
  ...
  if (displayGraph) {
    ...
    xRect.setAttribute('x', '48');
    xRect.setAttribute('y', (height - 24 - 1).toString(10));
    xRect.setAttribute('width', (width - 48).toString(10));
    xRect.setAttribute('height', '2');
    xRect.setAttribute('stroke', 'none');
    xRect.setAttribute('fill', baseColor);
  ...
  if (displayText) {
    ...
    [0, -10, -20, -30, -40, -50, -60].forEach((dB, index) => {
      ...
      text.setAttribute('y', (index * ((height - 2 * translateY) / 6) + translateY).toString(10));
      text.setAttribute('text-anchor', 'end');
      text.setAttribute('stroke', 'none');
      text.setAttribute('fill', baseColor);
      text.setAttribute('font-size', '12px');
  ...
  const render = () => {
    ...
    for (let k = 0; k < analyser.frequencyBinCount; k++) {
      ...
      const x = k * (innerWidth / analyser.frequencyBinCount) + translateX;
      const y = Math.min((0 - data[k]) * (height / (maxDecibels - minDecibels)) - translateY, height - translateY);
...
const animateAmplitudeSpectrumToCanvas = (canvas, button, displayGraph, displayText) => {
  ...
  const render = () => {
    ...
    if (displayGraph) {
      renderingContext.fillStyle = baseColor;
      renderingContext.fillRect(48, height - 24 - 1, innerWidth, 2);
      renderingContext.fillRect(48, 24, 2, height - 48);
    }
    ...
    if (displayText) {
      ...
      [0, -10, -20, -30, -40, -50, -60].forEach((dB, index) => {
        renderingContext.textAlign = 'end';
        renderingContext.fillText(`${dB} dB`, 44, index * ((height - 2 * translateY) / 6) + translateY);
      });
