...
const renderFrequencyResponse3BandsEqualizer = (svg) => {
  ...
  document.getElementById('range-equalizer-bass-gain').addEventListener('input', (event) => {
    bass.gain.value = event.currentTarget.valueAsNumber;

    document.getElementById('print-equalizer-bass-gain').textContent = `${bass.gain.value} dB`;

    render(bass);
  });

  document.getElementById('range-equalizer-middle-gain').addEventListener('input', (event) => {
    middle.gain.value = event.currentTarget.valueAsNumber;

    document.getElementById('print-equalizer-middle-gain').textContent = `${middle.gain.value} dB`;

    render(middle);
  });

  document.getElementById('range-equalizer-treble-gain').addEventListener('input', (event) => {
    treble.gain.value = event.currentTarget.valueAsNumber;

    document.getElementById('print-equalizer-treble-gain').textContent = `${treble.gain.value} dB`;
